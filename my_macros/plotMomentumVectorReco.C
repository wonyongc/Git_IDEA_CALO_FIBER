...
//                 std::cout << " input direction: phi = " << phi << " :: eta = " << eta << std::endl;                  
          ...
          double maxEneHitTiming = 7;
          long unsigned int maxTimingHits      = 4;
...
//   leg->Draw();
  ...
  hDeltaPhi_Barrel[NFILES-1]->GetYaxis()->SetRangeUser(0,hDeltaPhi_Barrel_wGravity[NFILES-1]->GetMaximum()*1.5);
...
//       leg->AddEntry(hDeltaPhi_Barrel_wGravity[iFile], Form("CoG:~~RMS_{#phi}=%.2f mrad",phi_res_b_cg*1000),  "lp");        
  ...
  hDeltaPhi_Endcap[NFILES-1]->GetYaxis()->SetRangeUser(0,hDeltaPhi_Endcap_wGravity[NFILES-1]->GetMaximum()*1.5);
